...
class RepositoryTest : FunSpec({
    ...
    test("Check if returns all sources") {
        githubProvider.repositories.first().getSources("java").count() shouldBeExactly EXPECTED_GH_REPO_SOURCES
        bitbucketProvider.repositories.first().getSources("java").count() shouldBeExactly EXPECTED_BB_REPO_SOURCES
    }
