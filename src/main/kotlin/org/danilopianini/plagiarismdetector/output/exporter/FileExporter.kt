 ...
 */
abstract class FileExporter<in M : Match>(private val outputDirectory: Path, private val output: Output) :
    ReportsExporter<M> {
    /**
     * Exports the given [reports] in a file inside [outputDirectory].
     */
    override operator fun invoke(reports: Set<Report<M>>) = reports.groupBy { it.submittedProject }
        .forEach {
